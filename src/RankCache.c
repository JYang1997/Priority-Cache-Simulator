...
uint8_t access(RankCache_t* cache, uint64_t key, uint32_t size) {
	...
	if (item == NULL) {//cache miss
		...
		if (size > cache->capacity) {
			...
			fprintf(stderr, "key: %ld size: %d exceed cache capacity! \n", key, size);
			exit(-1);
		}

		uint32_t index = cache->currNum;
...
uint32_t evictItem(RankCache_t* cache) {
	...
	while (1) { //eps needed
		rand_index = rand()%(cache->currNum); //randomly select one element
		...
		if (cache->currSize > cache->capacity) { //eps need
			...
			if (ret_index != cache->currNum) {
				temp2= NULL;
				HASH_FIND(pos_hh, cache->ItemIndex_HashTable, &(cache->currNum), sizeof(uint32_t), temp2);
				HASH_DELETE(pos_hh, cache->ItemIndex_HashTable, temp2);
				temp2->index = ret_index;
				HASH_ADD(pos_hh, cache->ItemIndex_HashTable, index, sizeof(uint32_t), temp2);
			}
		}else {
			break;
		}
